  ...
  if (binf!=NULL) {
    ...
    if (errnum==0 && strlen(errfname)==0) {
      ...
      if ((sc_debug & sSYMBOLIC)!=0 || verbosity>=2 || stacksize+32>=(long)pc_stksize || flag_exceed) {
        pc_printf("Размер заголовка:\t\t%8ld байт\n", (long)hdrsize);
        pc_printf("Размер кода:\t\t\t%8ld байт\n", (long)code_idx);
        pc_printf("Размер данных:\t\t\t%8ld байт\n", (long)glb_declared*sizeof(cell));
    ...
    do {                                /* there are arguments; process them */
      ...
      case '&':
        if (ident!=iVARIABLE || numtags>0)
          error(1,"-идентификатор-","&");
        ident=iREFERENCE;
        break;
      case tCONST:
        if (ident!=iVARIABLE || numtags>0)
          error(1,"-идентификатор-","const");
        fconst=TRUE;
        break;
      case tLABEL:
        if (numtags>0)
          error(1,"-идентификатор-","-тэг-");
